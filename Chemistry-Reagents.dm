...
/datum/reagent/pyroxadone
	name = "Pyroxadone"
	id = PYROXADONE
	description = "the spicy brother of the cryoxadone"
	reagent_state = REAGENT_STATE_LIQUID
	color = "#f7832a" 
	density = 1.47
	specheatcap = 3.47

/datum/reagent/pyroxadone/on_mob_life(var/mob/living/M)

	if(..())
		return 1

	if(M.bodytemperature > 360,15)
		M.adjustCloneLoss(-1)
		M.adjustOxyLoss(-1)
		M.heal_organ_damage(2,3)
		M.adjustToxLoss(-1)
